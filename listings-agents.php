...
$query_Recordset1 = "SELECT * FROM listings  LEFT JOIN (SELECT photoAlbums.albumID,photoAlbums.coverPhotoID,photoAlbums.albumName,photos.id,photos.file_name FROM photoAlbums,photos WHERE photoAlbums.coverPhotoID=photos.id)  AS a ON listings.albumID=a.albumID  WHERE listings.agentName = '".$_GET['agentName']."' AND listings.websiteID = ".$websiteID;
...
?>
<?php
$pageTitle = $_GET['agentName'].': Listings';
?>
...
<!-- InstanceBeginEditable name="mainContent" -->
  ...
  <div class="main">
	...
	if($totalRows_Recordset1==0){
		echo "There are currently no listings for this agent. Please check back later!";
	}
	else {
	?>
